...
export function parseRam(text = '') {
  const forward = [...text.matchAll(/(\d{1,3})\s*GB[^\n,;]{0,40}(?:DDR\d?|RAM|Arbeitsspeicher)/gi)].map(m => Number(m[1]));
  const reverse = [...text.matchAll(/(?:RAM|Arbeitsspeicher)[^\n,;]{0,30}?(\d{1,3})\s*GB/gi)].map(m => Number(m[1]));
  const values = [...forward, ...reverse];

...
export function evaluateAd(ad) {
  ...
  const baseRequirements = ad.active && silent.proven && price != null && cpuVideo.qualified && distanceVerified;
  const completeHardware = ram >= 16 && Boolean(storage) && displays.suitable;
  ...
  const classification = baseRequirements && completeHardware
    ? 'A'
    : baseRequirements && (ram >= 16 || upgradeCandidate) && openQuestions.length >= 1 && openQuestions.length <= 2
      ? 'B'
      : null;
  const matches = classification === 'A';
  return { ...ad, manufacturerModel: model, cpu, cpuVideoSuitability: cpuVideo.evidence, gpu, ramGb: ram, storage: storage?.label ?? 'nicht angegeben', upgradeable: upgrade.explicitlyUpgradable, recommendedUpgrades: upgrade.upgrades, upgradeCost: upgrade.cost, silentConcept: silent.concept, displaySuitability: displays.evidence, comparisonPrice, savingsBeforeUpgrades, savings, score: Math.max(0, Math.round(score * 10) / 10), classification, openQuestions, matches, priceLabel: price == null ? 'nicht angegeben' : euro.format(price) };
}

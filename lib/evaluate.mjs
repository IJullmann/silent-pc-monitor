...
export function evaluateAd(ad) {
  ...
  const classification = ad.active && silent.proven && ram >= 16 && Boolean(storage) && displays.suitable && price != null && distanceVerified
    ? 'A'
    : ad.active && silent.proven && price != null && cpu !== 'nicht angegeben' && (ram == null || ram >= 8) && openQuestions.length >= 1 && openQuestions.length <= 2
      ? 'B'
      : null;
  const matches = classification === 'A';
  return { ...ad, manufacturerModel: model, cpu, gpu, ramGb: ram, storage: storage?.label ?? 'nicht angegeben', silentConcept: silent.concept, displaySuitability: displays.evidence, comparisonPrice, savings, score: Math.round(score * 10) / 10, classification, openQuestions, matches, priceLabel: price == null ? 'nicht angegeben' : euro.format(price) };
}

...
function notificationBody(item) {
  ...
  return [`Klasse ${item.classification}: ${item.manufacturerModel}`, `${item.location} · ${item.distanceKm ?? 'Versand/unklar'} km`, `CPU: ${item.cpu}`, `Windows 11: ${item.windows11Suitability}`, `Video: ${item.cpuVideoSuitability}`, `GPU: ${item.gpu}`, `RAM: ${item.ramGb ?? 'nicht angegeben'} GB · Speicher: ${item.storage}`, `Aufrüstung: ${upgrades}`, `Silent: ${item.silentConcept}`, `3 Monitore: ${item.displaySuitability}`, `Zustand: ${item.condition} · Alter geschätzt: ${item.estimatedAge} J. · Garantie: ${item.warrantyMonths || 0} Mon.`, `Heutiger Neupreis: ca. ${item.replacementNewPrice} €`, `Zustands-/altersbereinigter Vergleichswert: ca. ${item.comparisonPrice} €`, `Ersparnis nach Aufrüstung: ${saving} · Verkäufer: ${item.sellerRating}`, `Preisbasis: ${item.priceBasisDate} · Score: ${item.score}/10`, item.url].join('\n');
}
